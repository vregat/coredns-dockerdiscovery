...
func (dd DockerDiscovery) ServeDNS(ctx context.Context, w dns.ResponseWriter, r *dns.Msg) (int, error) {
	log.Printf("[VREGAT] Serving DNS")
	state := request.Request{W: w, Req: r}
	var answers []dns.RR
	log.Printf("[VREGAT] QName: %s", state.QName())
	switch state.QType() {
	case dns.TypeA:
...
func (dd DockerDiscovery) getContainerAddress(container *dockerapi.Container) (net.IP, error) {
	...
	for {
		...
		// Use reverse proxy IP instead of docker network IP
		if dd.reverseProxy != "" {
			return net.ParseIP(dd.reverseProxy), nil
		}
		...
		if strings.HasPrefix(networkMode, "container:") {
			log.Printf("Container %s is in another container's network namespace", container.ID[:12])
...
func (dd DockerDiscovery) updateContainerInfo(container *dockerapi.Container) error {
	...
	if len(domains) > 0 {
		...
		if !isExist {
			...
			log.Printf("[docker] Add entry of container %s (%s). Public IP: %v. Domains: %s", normalizeContainerName(container), container.ID[:12], containerAddress, dm)
		}
	} else if isExist {
		log.Printf("[docker] Remove container entry %s (%s)", normalizeContainerName(container), container.ID[:12])
	}
